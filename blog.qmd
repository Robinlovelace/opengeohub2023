...
<!---
 jn: shorter title proposal: "Geographic data analysis in R and Python: comparision of working with vector datasets" 
--->
...
```{python}
poi_joined = poi_buffer.sjoin(stops, how='left')
# Column names:
poi_joined.columns
poi_joined[['stop_id', 'point'  ]]
```

## R

```{r}
poi_joined = st_join(poi_buffer, stops)
names(poi_joined)
poi_joined[c("stop_id", "geometry")]
```
