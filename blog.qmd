...
For buffer operations to work in Python you must reproject the data first (which we did, see above) (although there plans for **geopandas** to support a spherical geometry backend at some point, as discussed in issue [#2098](https://github.com/geopandas/geopandas/issues/2098)).
...
- R returns measures with units, which can make conversions easier, but which can also be confusing for new users
