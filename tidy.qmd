...
- [DuckDB](https://duckdb.org/docs/api/r.html), a high-performance in-memory database that can be used with `dplyr`
...
- [`tidypolars`](), an R interface to the [`polars`](https://github.com/pola-rs/polars) Rust crate (note: there is also a popular [Python interface](https://pola-rs.github.io/polars-book/user-guide/installation/)

How to assess which to use?
My advice would be to focus on solving the problem, rather than the tools.
